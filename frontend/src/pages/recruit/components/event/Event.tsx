...
export const RecruitEvent = () => {
  return (
    <div className="flex flex-col items-center bg-black py-20">
      ...
      <div className="grid w-full min-w-0 max-w-screen-lg grid-flow-col grid-rows-3 gap-4 px-10 py-10 sm:grid-rows-2 sm:px-16 md:px-20">
        {EVENTS_DETAIL.map(({ id, title, description }) => (
          <HoverCard key={id} title={title} description={description} />
        ))}
      </div>
    </div>
  )
